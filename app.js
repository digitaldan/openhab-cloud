...
io.use(function (socket, next) {
    ...
    Openhab.findOne({
        ...
        if (error) {
            ...
            if (openhab) {
                socket.openhab = openhab; // will use this reference in 'connection' to save on mongo calls
                next();
            } else {
...
io.sockets.on('connection', function (socket) {
    logger.info('openHAB-cloud: starting connection for uuid ' + socket.handshake.uuid);
    if(!socket.openhab){
    ...
    delete socket.openhab; //we really don't want to be using outdated model references from here on out.
